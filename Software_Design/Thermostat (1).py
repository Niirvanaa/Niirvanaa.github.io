...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## Explicit state transitions to prevent library cycle bugs
    ##
    to_heat = off.to(heat)
    to_cool = heat.to(cool)
    to_off = cool.to(off)
    ...
    def setupSerialOutput(self):
        output = f"{self.current_state.id},{self.getFahrenheit():.1f},{self.setPoint}\r\n"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%m/%d/%Y %H:%M").ljust(16)

            ## Setup Display Line 2
            if(altCounter < 6):
                lcd_line_2 = f"Temp: {self.getFahrenheit():.1f} F".ljust(16)

                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"St:{self.current_state.id} SP:{self.setPoint}F".ljust(16)
...
##
greenButton = Button(24, bounce_time=0.2)
